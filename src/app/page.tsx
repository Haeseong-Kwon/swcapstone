...
export default function Home() {
  ...
  // IntersectionObserver: activate scroll-reveal elements
  useEffect(() => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-50 overflow-x-hidden">
      ...
      <section className="relative flex min-h-[100svh] items-center overflow-hidden bg-black pt-20">
        <div className="absolute inset-0 z-0 overflow-hidden gpu-accelerated">
          <Image
          ...
          <div className="grid items-end gap-10 min-[1200px]:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)] lg:gap-12 xl:gap-20">
            ...
            <div className="animate-slide-up space-y-8 sm:space-y-12 [animation-delay:0.1s]">
              ...
              <div className="relative max-w-2xl animate-slide-up [animation-delay:0.3s] transform-gpu">
                <div className={cn(
                  "group flex flex-col gap-3 rounded-[2rem] border border-white/12 bg-black/40 p-3 shadow-[0_32px_100px_rgba(0,0,0,0.3)] sm:flex-row sm:items-center sm:gap-0 sm:rounded-full sm:p-2",
                  "transition-[box-shadow] duration-200",
                  searchFocused && "shadow-[0_0_0_3px_rgba(96,165,250,0.25),0_32px_80px_rgba(0,0,0,0.35)]"
                )}>
                  <div className="flex min-w-0 flex-1 items-center px-4 sm:px-8">
                    <Search className="mr-3 text-white/70 sm:mr-4 shrink-0" size={24} />
                    <input
                      type="text"
                      value={searchQuery}
                  ...
                  <button
                    ...
                    className="flex h-10 items-center justify-center gap-2 rounded-full bg-white px-5 text-[12px] font-bold uppercase tracking-[0.22em] text-slate-950 interactive hover:bg-primary hover:text-white sm:mx-2 sm:h-14 sm:px-10"
                ...
                <div className="mt-6 flex flex-wrap gap-2 px-2 sm:gap-4 sm:px-8">
                  {["#LMS", "#TeamBuilding", "#Capstone", "#Startup"].map((tag) => (
                    <span
                      key={tag}
                      className="rounded-full border border-white/10 bg-white/[0.08] px-3 py-1 text-[11px] font-bold text-white/60 interactive sm:px-4 sm:py-1.5 sm:text-[12px]"
                      style={{ transition: 'border-color 0.15s, color 0.15s, background-color 0.15s' }}
                      onMouseEnter={(e) => {
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 text-white/70 sm:flex sm:flex-wrap sm:gap-8">
                {[
                  ...
                  <div
                    key={title}
                    className="rounded-2xl border border-white/10 bg-white/8 px-6 py-6 sm:min-w-[200px] sm:flex-1"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 content-auto w-full animate-slide-up [animation-delay:0.5s]">
              {featureCards.map((card) => (
                <Link
                  key={card.title}
                  href={card.href}
                  className="group relative flex flex-col justify-between overflow-hidden rounded-[2rem] border border-white/10 bg-white/8 p-6 shadow-xl sm:rounded-[3rem] sm:p-10 md:h-[260px] lg:h-[280px] interactive"
                  style={{
                    transition: 'background-color 0.2s ease-out, border-color 0.2s ease-out, transform 0.24s cubic-bezier(0.34,1.56,0.64,1)',
                  }}
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLElement).style.backgroundColor = 'rgba(255,255,255,0.12)';
                  ...
                  onMouseLeave={(e) => {
                    ...
                    (e.currentTarget as HTMLElement).style.transform = 'translateY(0) translateZ(0)';
                  }}
                >
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 text-white icon-bounce sm:h-16 sm:w-16"
                      style={{ transition: 'background-color 0.15s, color 0.15s' }}
                    >
                      {card.icon}
                    </div>
                    <div className="hidden h-10 w-10 items-center justify-center rounded-full bg-white/10 sm:flex sm:h-12 sm:w-12"
                      style={{ transition: 'background-color 0.15s, transform 0.2s cubic-bezier(0.34,1.56,0.64,1)' }}
                    >
      ...
      {/* CORE ECOSYSTEM */}
      <section className="bg-white section-padding dark:bg-slate-900 overflow-hidden contain-layout-paint">
        <div className="fluid-container transform-gpu">
          <div className="section-accent-bar mb-12 sm:mb-20 scroll-reveal">
          ...
          <div className="grid grid-cols-1 border-l border-t border-slate-200 dark:border-slate-700 md:grid-cols-2 min-[1200px]:grid-cols-3">
            {[
              {
              ...
              <div
                key={item.title}
                className="group relative overflow-visible border-b border-r border-slate-200 p-10 dark:border-slate-700 sm:p-14 lg:p-16 xl:p-20 scroll-reveal"
              ...
              >
                ...
                <h4 className="mb-6 text-[32px] font-black uppercase tracking-[-0.05em] text-slate-900 dark:text-slate-50 lg:text-[42px]"
